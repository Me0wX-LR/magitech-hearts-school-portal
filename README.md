...
自創學派寫在 master 第 20 列起。角色卡學派下拉改成 `'學派表'!$A$2:$A$200`。

Master 請設「知道連結的人可**檢視**」；審核者可編輯。玩家不必進試算表。

## 4. GM 審核（master）

1. 打開 **申請** 工作表，看新列（右方有驗證結果／剩餘功績點／藏書與特記草稿）。
2. 不要的列：把 **驗證結果** 改成 `退回`。
3. 要過的那一列：點該列任一格，選單 **學派統合 → 發布目前選取的一列**。
4. 或批次：
   - **發布所有待審的初創申請**（只收驗證訊息＝通過的初創）
   - **發布所有待審的運營申請**（升級／加點／追加藏書／特記）

發布後會寫入 **學派表／學派卡**。自動角卡下一輪 `IMPORTRANGE` 就會看到。

玩家升級走入口的 [運營／升級](ops.html)，不要再填初創表。運營頁直接讀 master「學派卡」（gviz），不依賴 Web App 的 `action=cards`。

貼上新的 `Code.gs` 後請 **儲存**，並 **部署 → 管理部署 → 編輯 → 新版本**，否則送出申請會失敗。
